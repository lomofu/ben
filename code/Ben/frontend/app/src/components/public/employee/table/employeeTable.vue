    ...
    <v-card outlined max-width="98%" class="ma-2" flat>
      ...
      >
        ...
        <template v-slot:item.op="{ item }">
          <Permission
            v-if="!item.admin && !onlyChose"
            :perm="getEditEmployeePerm"
          >
            <v-btn
              v-if="!item.admin"
              small
...
export default {
  ...
  components: {
    ...
    search: () => import("../search/searchForAccount.vue")
  },
  data: () => ({
    _timer: null,
    showColor: "amber",
    ...
    headers: [
      ...
      { text: "操作", value: "op", align: "center", sortable: false }
    ],
    tempAccount: null
  }),
  methods: {
    ...
    handleChangeRole(item) {
      eventBus.$emit("showChangeRole", item);
    },
    updateSearch(val) {
      this.selected = [];
    ...
    cancel() {
      this.$store.commit("updateEmployeeSearchAll", true);
  ...
  computed: {
    ...
    getSearchAll() {
      return this.$store.getters.getEmployeeSearchAll;
    },
    showSelect() {
      return (
